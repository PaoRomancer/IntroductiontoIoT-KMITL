...
const char* ssid = "aungpao"; //อย่าลืมแก้ไข SSID ของ WIFI ที่จะให้ NodeMCU ไปเชื่อมต่อ
const char* password = "aungpaoaungpao"; //อย่าลืมแก้ไข PASSWORD ของ WIFI
const char* mqtt_server = "broker.hivemq.com"; //อย่าลืมแก้ไข BROKER
const int mqtt_port = 1883;
const char* mqtt_Client = "fa54c3d5-205c-4eaa-8695-Natthawit";  //อย่าลืมแก้ไข ClientID
...
void reconnect() {
  ...
  if (client.connect(mqtt_Client, mqtt_username, mqtt_password)) { //เชื่อมต่อกับ MQTT BROKER
    ...
    client.subscribe("Natthawit/led/status"); //แก้
  }
  else {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(String(topic) == "Natthawit/led/status") { //แก้
    if (message == "ON"){
...
void setup() {
  ...
  client.subscribe("Natthawit/led/status");
}
...
void loop() {
  ...
  client.loop();
  long now = millis();
  if (now - lastMsg > 2500) { //จับเวลาส่งข้อมูลทุก ๆ 5 วินาที
    lastMsg = now;
    ++value;
    ...
    Serial.println(msg);
    client.publish("natthawit/data", msg); // อย่าลืมแก้ไข TOPIC ที่จะทำการ PUBLISH ไปยัง MQTT BROKER
  }
  delay(1);
}